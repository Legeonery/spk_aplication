...
const fetchEligibleVehicles = async () => {
  try {
    ...
    vehicles.value = res.data.filter(
      (v) => ['привоз', 'отгрузка', 'универсальный'].includes(v.type) && !v.is_available,
    )
  } catch (e) {
...
watch(selectedVehicleId, async (id) => {
  ...
  maxWeight.value = vehicle?.max_weight ?? null

  try {
    const res = await api.get('/tare-measurements/check', {
...
const submit = async () => {
  ...
  if (maxWeight.value && tareWeight.value > maxWeight.value) {
    return (error.value = `Вес тары (${tareWeight.value} кг) превышает допустимый вес ТС (${maxWeight.value} кг)`)
  }
  loading.value = true
  try {
    ...
    <div v-if="show" class="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center">
      <div class="bg-white p-6 rounded-xl w-full max-w-md shadow-xl relative animate-fade-in">
        ...
        <div class="space-y-4">
          ...
          <input type="number" v-model="tareWeight" placeholder="Введите вес тары, кг"
