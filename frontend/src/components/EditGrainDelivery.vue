...
const submit = async () => {
  try {
    await api.put(`/grain-deliveries/${form.value.id}`, form.value)
    emit('success')
    emit('close')
  } catch (err) {
      ...
      <div class="bg-white rounded-xl shadow p-6 w-full max-w-md relative animate-fade-in">
        ...
        <div class="space-y-3">
          ...
          <div class="flex justify-end mt-2">
            <button @click="submit" class="bg-blue-600 text-white px-4 py-2 rounded">💾 Сохранить</button>
          </div>
        </div>
      </div>
    </div>
