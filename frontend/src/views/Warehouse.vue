...
<template>
  <div class="p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
      ...
      <!-- Список складов -->
      <router-link v-for="warehouse in warehouses" :key="warehouse.id" :to="`/warehouse/${warehouse.id}`"
        class="p-4 flex flex-col items-center justify-center border rounded-xl shadow hover:shadow-md hover:bg-gray-50 transition">
        <img :src="warehouse.type === 'зерновой' ? '../../public/wheat (1).png' : '../../public/wheat_component.png'"
        ...
        <span v-if="warehouse.type === 'зерновой' && warehouse.max_historical_load > 0" class="text-gray-500 text-sm">
